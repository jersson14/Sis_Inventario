...
switch ($op) {
		...
		if ($rs) {
			while ($reg = $rs->fetch_object()) {
				...
				if ($reg->estado === 'PENDIENTE') {
					...
					$btn .= '<button class="btn btn-default btn-xs" title="Marcar aceptada" onclick="cambiarEstado(' . $id . ',\'ACEPTADA\')"><i class="fa fa-thumbs-o-up"></i></button> ';
	...
	case 'mostrar':
		...
		if ($rs) {
			while ($d = $rs->fetch_assoc()) {
				$c['items'][] = array('idarticulo' => (int)$d['idarticulo'], 'nombre' => $d['nombre'], 'codigo' => $d['codigo'], 'unidad' => $d['unidad'], 'stock' => (int)round((float)$d['stock']), 'cantidad' => (int)round((float)$d['cantidad']), 'precio' => round((float)$d['precio'], 2), 'descuento' => round((float)$d['descuento'], 2), 'subtotal' => round((float)$d['subtotal'], 2));
			}
		}
		responderJson($c);
		break;
	...
	case 'listarArticulos':
		...
		if ($rs) {
			while ($reg = $rs->fetch_object()) {
				...
				$pill = $stock <= 0 ? 'stock-empty' : ($stock <= max((int)round((float)$reg->stock_minimo), 5) ? 'stock-low' : 'stock-ok');
				$btn = '<button class="btn btn-add-item" type="button" onclick="agregarDetalle(' . (int)$reg->idarticulo . ',\'' . addslashes((string)$reg->nombre) . '\',' . $precio . ',\'' . addslashes((string)$reg->abreviatura) . '\',' . $stock . ')"><i class="fa fa-plus-circle"></i> Agregar</button>';
				...
				$data[] = array('0' => $btn, '1' => e($reg->nombre), '2' => e($reg->categoria), '3' => e($reg->abreviatura), '4' => e($reg->codigo), '5' => '<span class="stock-pill ' . $pill . '">' . $stock . '</span>', '6' => formatearMoneda($precio), '7' => $img);
			}
		}
		echo json_encode(array('sEcho' => 1, 'iTotalRecords' => count($data), 'iTotalDisplayRecords' => count($data), 'aaData' => $data), JSON_UNESCAPED_UNICODE);
		break;
	...
	case 'buscarArticuloCodigo':
		require_once "../modelos/Articulo.php";
		$articulo = new Articulo();
		$reg = $articulo->buscarActivoPorCodigo(isset($_POST['codigo']) ? limpiarCadena($_POST['codigo']) : '');
		if (!$reg) { responderJson(array('ok' => false, 'message' => 'No se encontró un artículo con ese código')); }
		responderJson(array('ok' => true, 'idarticulo' => (int)$reg['idarticulo'], 'nombre' => $reg['nombre'], 'precio_venta' => (float)$reg['precio_venta'], 'unidad' => $reg['abreviatura'], 'stock' => (int)round((float)$reg['stock'])));
		break;
