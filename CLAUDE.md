...
5. **Stock:** los triggers `tr_updStockIngreso` / `tr_udpStockVenta` mueven stock al insertar detalles. Anular venta/ingreso debe revertir stock explícitamente (ya lo hacen los modelos). Los ajustes (`ajuste_inventario`) actualizan stock desde el modelo `Inventario`. Todo kardex debe unir ingresos + ventas + ajustes.
