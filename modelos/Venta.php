...
class Venta{
	...
	public function insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$tipo_pago,$medio_pago,$fecha_vencimiento,$observacion,$idarticulo,$cantidad,$precio_venta,$descuento,$idpresentacion = array(),$idvariante = array(),$pago = array()){
		...
		$resultado = dbTransaccion(function($cx) use ($ctx, $detalles, $cantidadesSolicitadas, $cantidadesVariante, &$mensajeError) {
			...
			foreach ($cantidadesSolicitadas as $idArt => $cantSolicitada) {
				...
				$stockDisp = round((float)$stockActual[$idArt]["stock"], 3);
				// Con control de vencimientos, lo vencido no esta disponible para la venta
				$vencido = Lote::activo() ? Lote::stockVencido($idArt) : 0.0;
			...
			foreach ($detalles as $d) {
				...
				if ($usaLotes) {
					...
					if (Lote::consumir($d["idarticulo"], round($d["cantidad"] * $d["factor"], 3), $ref) === false) {
	...
	public function eliminar($idventa, $idusuario){
		...
		$resultado = dbTransaccion(function($cx) use ($idventa, &$mensajeError, &$documento) {
			...
			if (!dbExec("DELETE FROM caja_movimiento WHERE referencia IN (?,?)", array("V-" . $idventa, "AV-" . $idventa))) {
